...
# ============================================
# PART 1: EFFICIENT DATA LOADING WITH DUCKDB
# ============================================
...
cat("Loading Medicare_Data.csv into DuckDB...\n")
dbExecute(con, "CREATE TABLE medicare_data AS SELECT * FROM read_csv_auto('Medicare_Data.csv')")

# Query Texas data - USING EXACT COLUMN NAMES FROM YOUR DATASET
cat("\nQuerying Texas data...\n")
...
# Check if we got Texas data
cat("\nTexas data rows:", nrow(texas_data), "\n")
...
cat("\nFinal data rows:", nrow(texas_data), "\n")
...
analysis_data <- texas_data %>%
  ...
  mutate(
    ...
    # Facility type based on place of service
    is_facility = ifelse(place_of_service %in% c("21", "22", "23", "24"), 1, 0),
    
    # Provider specialty based on provider type
...
# ============================================
# PART 3: AGGREGATE FOR CLUSTERING
# ============================================

# Aggregate at procedure level
procedure_metrics <- analysis_data %>%
  group_by(hcpcs_code, hcpcs_description) %>%
  summarise(
    ...
    avg_revenue_per_beneficiary = mean(revenue_per_beneficiary, na.rm = TRUE),
    provider_count = n_distinct(npi),
    .groups = 'drop'
  ) %>%
  mutate(
    revenue_per_unit = total_revenue / total_volume,
    efficiency_score = avg_payment_to_charge * revenue_per_unit / 100
  ) %>%
  filter(total_volume >= 50)

cat("\nProcedure metrics rows:", nrow(procedure_metrics), "\n")
...
cat("FILES GENERATED:\n")
cat("1. pricing_alignment_by_specialty.png - Chart by provider type\n")
cat("2. top_20_procedures.png - Top revenue procedures\n")
cat("3. pareto_analysis.png - Revenue concentration analysis\n")
cat("4. cluster_analysis.png - Procedure segmentation\n")
cat("5. simulation_results.png - Revenue impact scenarios\n")
cat("6. specialty_pricing_analysis.csv - Pricing data by specialty\n")
cat("7. top_20_procedures.csv - Top procedures list\n")
cat("8. top_50_procedures.csv - Extended top procedures\n")
cat("9. cluster_summary.csv - Cluster interpretations\n")
cat("10. simulation_results.csv - Scenario analysis\n")
cat("11. analysis_data.rds - Cleaned dataset\n")
cat("12. procedure_metrics.rds - Procedure-level metrics\n")
cat("\n")
cat("================================================================\n")
cat("Ready for presentation preparation!\n")
cat("================================================================\n")
